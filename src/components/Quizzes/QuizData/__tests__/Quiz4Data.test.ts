import { Quiz4Data, Quiz4DataTitle } from "../Quiz4Data";

describe("Quiz4Data", () => {
    test("is an array", () => {
        expect(Array.isArray(Quiz4Data)).toBe(true);
    });

    test("has correct title", () => {
        expect(Quiz4DataTitle).toBe("Quiz 4: Mad Libs");
    });

    test("has correct structure", () => {
        Quiz4Data.forEach((slide) => {
    ...
    test("slide numbers are in correct order", () => {
        for (let i = 0; i < Quiz4Data.length - 1; i++) {
            expect(Quiz4Data[i].QuestionNumber + 1).toBe(
                Quiz4Data[i + 1].QuestionNumber
            );
        }
    });
