...
char operation;`,
    },
    {
        ...
        disc: ` To accept user input, we use the <span class="style-html">std::cin</span> object. When you use <span class="style-html">std::cin >> (variable-name-here);</span>, it waits for the user to enter something and then stores the input into the specified variable. We will use this in our calculator program to capture the numbers and the operation that the user wants to perform. This is EXACTLY what we did in the previous project. I just thought a refresher would be nice!`,
        code:
`std::cout << "Enter first number: ";
std::cin >> num1;`
    },
...
`
...

    {
        SlideNumber: 7,
        SlideTitle: "std:endl",
        disc: `In C++, the <span class="style-html">std:endl</span> is an output manipulator used to insert a newline character into the output and flush the output buffer. It is commonly used with <span class="style-html">std::cout</span> to end the current line and move the cursor to the start of the next line in the console. For instance, <br></br> <span class="style-html">std::cout << "Hello, World!" << std::endl;</span> <br></br> would print the message <span class="style-html">Hello, World!</span> to the console and then start a new line. This is especially useful when you want to separate different pieces of output, or when you want to ensure that subsequent output appears on a new line. <br></br>
        
        You might be asking what the difference between <span class="style-html">\\n</span> and <span class="style-html">std::endl</span> is. In C++, both <span class="style-html">\\n</span> and <span class="style-html">std::endl</span> are used to insert a new line when displaying output to the console. You can use<span class="style-html">\\n</span> within a string that's being printed. On the other hand, <span class="style-html">std::endl</span> is typically used outside of strings and directly with the <span class="style-html">std::cout</span> stream, like so: <br></br> <span class="style-html">std::cout << "Hello" << std:endl << "World";</span> <br></br> I wouldn't worry too much about this for now.`,
        code:
`if (operation == '+') {
    std::cout << "The result is: " << num1 + num2 << std::endl;
}`
