import { Quiz1Data, Quiz1DataTitle } from "../Quiz1Data";

describe("Quiz1Data", () => {
    test("is an array", () => {
        expect(Array.isArray(Quiz1Data)).toBe(true);
    });

    test("has correct title", () => {
        expect(Quiz1DataTitle).toBe("Quiz 1: Introduction To C++");
    });

    test("has correct structure", () => {
        Quiz1Data.forEach((slide) => {
    ...
    test("slide numbers are in correct order", () => {
        for (let i = 0; i < Quiz1Data.length - 1; i++) {
            expect(Quiz1Data[i].QuestionNumber + 1).toBe(
                Quiz1Data[i + 1].QuestionNumber
            );
        }
    });
