import { Quiz6Data, Quiz6DataTitle } from "../Quiz6Data";

describe("Quiz6Data", () => {
    test("is an array", () => {
        expect(Array.isArray(Quiz6Data)).toBe(true);
    });

    test("has correct title", () => {
        expect(Quiz6DataTitle).toBe("Quiz 6: Guessing Game");
    });

    test("has correct structure", () => {
        Quiz6Data.forEach((slide) => {
    ...
    test("slide numbers are in correct order", () => {
        for (let i = 0; i < Quiz6Data.length - 1; i++) {
            expect(Quiz6Data[i].QuestionNumber + 1).toBe(
                Quiz6Data[i + 1].QuestionNumber
            );
        }
    });
