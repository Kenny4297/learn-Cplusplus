import { Quiz7Data, Quiz7DataTitle } from "../Quiz7Data";

describe("Quiz7Data", () => {
    test("is an array", () => {
        expect(Array.isArray(Quiz7Data)).toBe(true);
    });

    test("has correct title", () => {
        expect(Quiz7DataTitle).toBe("Quiz 7: Next Steps");
    });

    test("has correct structure", () => {
        Quiz7Data.forEach((slide) => {
    ...
    test("slide numbers are in correct order", () => {
        for (let i = 0; i < Quiz7Data.length - 1; i++) {
            expect(Quiz7Data[i].QuestionNumber + 1).toBe(
                Quiz7Data[i + 1].QuestionNumber
            );
        }
    });
