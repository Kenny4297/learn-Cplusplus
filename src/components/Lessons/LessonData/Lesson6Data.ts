...
`,
    },
    {
        ...
        disc: `<span class="style-html">srand(static_cast<unsigned int>(time(0)));</span> is a line of code that sets the seed for the random number generator in C++. <span class="style-html">srand</span> stands for "seed random", and it is a function that takes an unsigned integer as an argument to set the seed. If you use the same seed every time, the sequence of random numbers generated will always be the same. To avoid this, we typically use the current time as the seed because it changes every second, making it a good source of pseudo-randomness. The <span class="style-html">time()</span> function returns the current time, and <span class="style-html">static_cast<unsigned int></span> is used to convert this time value to an unsigned integer, which is the type of data required by the <span class="style-html">srand</span> function.`,
        code:        
